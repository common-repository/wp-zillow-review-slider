...
<div class="wpzillow_margin10">
	<a id="wpzillow_helpicon" class="wpzillow_btnicononly button dashicons-before dashicons-editor-help"></a>
	<a id="wpzillow_removeallbtn" class="button dashicons-before dashicons-no"><?php _e('Remove All Reviews', 'wp-zillow-reviews'); ?></a>
<p>
	<?php 
_e('Search reviews, hide certain reviews, manually add reviews, save a CSV file of your reviews to your computer, and more features available in the <a href="?page=wp_zillow-get_pro">Pro Version</a> of this plugin!', 'wp-zillow-reviews'); 
?>
</p>
</div>
<?php 
	...
	$checkorderby = sanitize_key($_GET['sortby']);
		...
		<table class="wp-list-table widefat striped posts">
			<thead>
				<tr>
					<th scope="col" style="min-width:70px" class="manage-column"><a href="'.esc_url( add_query_arg( 'sortby', 'reviewer_name',$currenturl ) ).$sortdirection.'"><i class="dashicons dashicons-sort '.$sorticoncolor[1].'" aria-hidden="true"></i> '.__('Name', 'wp-zillow-reviews').'</a></th>
					<th scope="col" width="70px" class="manage-column"><a href="'.esc_url( add_query_arg( 'sortby', 'rating',$currenturl ) ).$sortdirection.'"><i class="dashicons dashicons-sort '.$sorticoncolor[2].'" aria-hidden="true"></i> '.__('Rating', 'wp-zillow-reviews').'</a></th>
					<th scope="col" class="manage-column">'.__('Review Text', 'wp-zillow-reviews').'</th>
					<th scope="col" width="100px" class="manage-column"><a href="'.esc_url( add_query_arg( 'sortby', 'created_time_stamp',$currenturl ) ).$sortdirection.'"><i class="dashicons dashicons-sort '.$sorticoncolor[3].'" aria-hidden="true"></i> '.__('Date', 'wp-zillow-reviews').'</a></th>
					<th scope="col" width="70px" class="manage-column"><a href="'.esc_url( add_query_arg( 'sortby', 'review_length',$currenturl ) ).$sortdirection.'"><i class="dashicons dashicons-sort '.$sorticoncolor[4].'" aria-hidden="true"></i> '.__('Length', 'wp-zillow-reviews').'</a></th>
					<th scope="col" width="100px" class="manage-column"><a href="'.esc_url( add_query_arg( 'sortby', 'pagename',$currenturl ) ).$sortdirection.'"><i class="dashicons dashicons-sort '.$sorticoncolor[5].'" aria-hidden="true"></i> '.__('Page Name', 'wp-zillow-reviews').'</a></th>
					<th scope="col" width="100px" class="manage-column"><a href="'.esc_url( add_query_arg( 'sortby', 'type',$currenturl ) ).$sortdirection.'"><i class="dashicons dashicons-sort '.$sorticoncolor[6].'" aria-hidden="true"></i> '.__('Type', 'wp-zillow-reviews').'</a></th>
				</tr>
				</thead>
		...
		if($reviewtotalcount>0){
			foreach ( $reviewsrows as $reviewsrow ) 
			{
				...
				$html .= '<tr>
						<th colspan="9" scope="col" class="manage-column">'.__('No reviews found. Please visit the <a href="?page=wp_zillow-get_zillow">Get Zillow Reviews</a> page to retrieve reviews.', 'wp-zillow-reviews').'</th>
					</tr>';
		}					
		...
		$html .= '<div id="wpzillow_review_list_pagination_bar">';
