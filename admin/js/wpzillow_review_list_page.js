(function( $ ) {
	...
	$(function(){
		...
		//help button clicked
		$( "#wpzillow_helpicon" ).click(function() {
		...
		//remove all button
		$( "#wpzillow_removeallbtn" ).click(function() {
		  openpopup("Are you sure?", '<p>This will delete all reviews in your Wordpress database including the ones you manually entered. It Does NOT affect your reviews on Facebook.</p>', '<a class="button dashicons-before dashicons-no" href="?page=wp_zillow-reviews&opt=delall">Remove</a>');
		});	

		//upgrade to pro
		$( ".wpzillow_upgrade_needed" ).click(function() {
		  openpopup("Upgrade Needed", '<p>Please upgrade to the Pro Version of this Plugin to access this feature.</p>', '<a class="button dashicons-before  dashicons-cart" href="?page=wp_zillow-get_pro">Upgrade Here</a>');
		});		
		...
		$( "#wpzillow_addnewreviewbtn" ).click(function() {
		  jQuery("#wpzillow_new_review").show("slow");
		});	
		$( "#wpzillow_addnewreview_cancel" ).click(function() {
		  jQuery("#wpzillow_new_review").hide("slow");
		  //reload page without taction and tid
		  setTimeout(function(){ 
			window.location.href = "?page=wp_zillow-reviews"; 
		  }, 500);
		...
		$('#upload_avatar_button').click(function() {
			tb_show('Upload Reviewer Avatar', 'media-upload.php?referer=wp_zillow-reviews&type=image&TB_iframe=true&post_id=0', false);
			return false;
		});
		...
		window.send_to_editor = function(html) {
			...
			$('#wpzillow_nr_avatar_url').val(image_url);
		...
		$("#newreviewform").submit(function(){ 
			  ...
			  if ($('input[name=wpzillow_nr_rating]:checked').length) {
			...
			if(jQuery( "#wpzillow_nr_name").val()==""){
				alert("Please enter a name.");
				$( "#wpzillow_nr_name" ).focus();
				return false;
			} else {
				return true;
			}
		...
		$("#review_list").on("click", ".reveditbtn", function (event) {
			...
			$("#wpzillow_new_review").show("slow");
			//find values from rowobject and fill in edit form wpzillow_nr_rating, wpzillow_nr_text, wpzillow_nr_name, wpzillow_nr_avatar_url, wpzillow_nr_date
			$("#editrid").val(rid);
			$("#wpzillow_nr_name").val(name);
			$("#wpzillow_nr_avatar_url").val(wprev_row_userpic);
			//for radio
			$("#wpzillow_nr_rating").val(wprev_row_rating);
			$("#wpzillow_nr_date").val(wprev_row_created_time);
			
			$("#wpzillow_nr_text").val(wprev_row_review_text);
		...
		function sendtoajax(rid,whattodo,rowobject){
			var senddata = {
					action: 'wpzillow_hide_review',	//required
		...
		//for pagination bar-----------------------------------
		$("#wpzillow_review_list_pagination_bar").on("click", "span", function (event) {
		...
		function sendtoajaxreview(pageclicked,sortbyval,sortd,selrevs){
			...
			$( "#review_list" ).html("");
			$( "#wpzillow_review_list_pagination_bar" ).html("");
			var senddata = {
				...
				var reviewtext = '';
					...
					if(numpages>1){
						...
						$( "#wpzillow_review_list_pagination_bar" ).html(pagebarhtml);

					if(reviewtotalcount==0){
						$("#wpzillow_review_list_pagination_bar").hide();
					} else {

						$("#wpzillow_review_list_pagination_bar").show();
					}
