 ...
 */
class WP_Zillow_Review_Activator {
	...
	public static function activate() {
		...
		$paidoptions = get_option( 'wpzillow_options' );
		$freeoptions = get_option( 'wpfbr_options' );
		if(!$paidoptions && $freeoptions){
			update_option( 'wpzillow_options', $freeoptions );
		}
		

		//setup cron to get zillow once a day
		if (! wp_next_scheduled ( 'wpzillow_daily_event' )) {
			wp_schedule_event(time(), 'daily', 'wpzillow_daily_event');
		}
	}
